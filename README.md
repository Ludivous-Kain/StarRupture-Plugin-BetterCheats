...
The grid and the fit setting are saved per session. Steppers and `bc_invsize <columns> <rows>` only queue the wanted size — press **Tab** or **E** (even with other keys held) to apply it immediately, then every tick for 3 seconds. Pressing either key again restarts that window.
...
These keys can be rebound at runtime via the modloader's keybind settings, and from the **Player -> Movement** panel -- either way the change takes effect immediately and is written back to the .ini.
